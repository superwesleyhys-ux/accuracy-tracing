...
- `h01`: both arms returned `ordinary`. The packet had no correction, anomaly,
  investigation or other specific pre-cutoff warning. The later finding was not
  recoverable from the supplied historical evidence without guessing.
- `h05`: the direct arm returned `elevated`; the harness returned `ordinary`.
  The harness treated the 2022 public image-manipulation allegation as one
  unconfirmed moderate signal. Its fixed threshold required one documented
  strong signal or two moderate signals, so it suppressed this alert.

Changing that threshold after seeing these labels would overfit the holdout. It
must be developed on separate cases and evaluated on a new untouched holdout.
