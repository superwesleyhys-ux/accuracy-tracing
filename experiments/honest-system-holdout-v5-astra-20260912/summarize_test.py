...
def main() -> None:
    ...
    def format_tokens(value: object) -> str:
        return f"{value:,}" if type(value) is int else "unavailable"

    def format_ratio(value: object) -> str:
        return f"{value:.2f}x" if isinstance(value, (int, float)) else "unavailable"
    ...
    lines = [
        ...
        f"| Input + output tokens | {format_tokens(direct['total_tokens'])} | {format_tokens(harness['total_tokens'])} |",
        f"| Harness / direct tokens | 1.00x | {format_ratio(token_ratio)} |", "",
