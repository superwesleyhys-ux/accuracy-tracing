#!/usr/bin/env python3
"""Run both frozen local arms without loading the sealed outcome labels."""
...
def validate_setup(corpus: dict, registration: dict) -> None:
    ...
    for case in cases:
        packet = case.get("public_packet")
        if not isinstance(packet, dict) or packet.get("case_id") != case.get("id"):
            raise SystemExit("case has an invalid public packet")
        if set(packet) != {"case_id", "question", "cutoff", "paper_profile", "evidence_passages"}:
            raise SystemExit("public packet fields are not fixed")
        passage_ids = [item.get("id") for item in packet["evidence_passages"]]
        if passage_ids != ["p001", "p002", "p003", "p004"]:
            raise SystemExit("each packet must contain the four fixed passage IDs")
...
def main() -> None:
    ...
    for step in schedule:
        ...
        packet = cases[case_id]["public_packet"]
