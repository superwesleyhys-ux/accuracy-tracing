...
- No remote git repository is configured in this delivery. No private UCA code is needed or modified.
...
Implement `TraceProvider.search` from `TRACE_ADAPTER.md`. Supply exact version identity, full text, observation time and a documented availability basis. Keep retrieval result ranking and following citations separate. Never use a search snippet as if it were the complete original. Count calls, retries, wall time and returned bytes; implement network deadlines and provider-specific limits in this boundary.

Acceptance: source text can be reopened; all spans refer to the stored version; revisions retain identity history; availability unknown remains unknown; network failure leaves usable audit output. Use public or explicitly connected data. This delivery has no connected live-news provider or model API credentials.

### B. Structured semantic decomposition adapter

Implement the `Decomposer` protocol with structured output conforming to `Analysis`. Model inputs contain target, eligible evidence, relevant current analyses, relations and gaps; do not include gold labels. Return exact spans, qualifiers, typed candidate relations, explicit gap resolutions and old versions to revisit. Do not invent spans for a paraphrase; link it to an actual source passage and distinguish analysis text from original text.
...
### D. Evidence verification adapter

Implement `Verifier.verify`. Evaluate semantic entailment, contradiction, temporal relevance and shared-source dependence at the claim level. An official source can be an original for its own announcement without making every claim in it true. Require basis spans for definite verdicts; emit verification gaps when evidence is missing. Retrieve further material only through the provider and mandatory decomposition route.
